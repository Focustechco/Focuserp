...
import { 
  ...
  LayoutGrid, List, MapPin, CreditCard, PowerOff, CheckCircle2,
  UserX, UserCheck
} from 'lucide-react';
...
export function FornecedoresList() {
  ...
  const { ativos, inativos } = useMemo(() => {
    const act: Fornecedor[] = [];
    const inact: Fornecedor[] = [];
    filteredData.forEach(f => {
      if (f.status === 'Inativo' || f.status === 'inativo') {
        inact.push(f);
      } else {
        act.push(f);
      }
  ...
  const handleToggleStatus = async (fornecedor: Fornecedor, e?: React.MouseEvent) => {
    ...
    const newStatus = fornecedor.status === 'Inativo' ? 'Ativo' : 'Inativo';
    try {
      updateItem(fornecedor.id, { status: newStatus });
      await fornecedorService.saveFornecedor({
        ...fornecedor,
        status: newStatus,
      } as any);

      if (newStatus === 'Inativo') {
        toast.info(`Fornecedor "${fornecedor.nomeFantasia || fornecedor.razaoSocial}" inativado e movido para a lista de Inativos.`);
      } else {
        toast.success(`Fornecedor "${fornecedor.nomeFantasia || fornecedor.razaoSocial}" reativado com sucesso!`);
      }
    } catch (err: any) {
      toast.error(`Erro ao alterar status: ${err?.message || 'Falha na operação'}`);
    }
  };
  ...
  const renderFornecedorCard = (fornecedor: Fornecedor, isInactiveSection = false) => {
    ...
    const isInactive = fornecedor.status === 'Inativo' || isInactiveSection;
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              <Badge 
                ...
                onClick={(e) => handleToggleStatus(fornecedor, e)}
                ...
                title={isInactive ? 'Clique para reativar fornecedor' : 'Clique para inativar fornecedor'}
              >
                {isInactive ? 'Inativo' : (fornecedor.status || 'Ativo')}
              </Badge>
  ...
  const renderFornecedorRow = (fornecedor: Fornecedor, isInactiveSection = false) => {
    const contatos = Array.isArray(fornecedor?.contatos) ? fornecedor.contatos : [];
    const contatoPrincipal = contatos.find(c => c?.principal) || contatos[0];
    const isInactive = fornecedor.status === 'Inativo' || isInactiveSection;
    ...
    return (
      <TableRow 
        key={fornecedor.id} 
        onClick={() => handleOpenPerfil(fornecedor)}
        className={`group cursor-pointer transition-colors ${isInactive ? 'opacity-75 hover:opacity-100 bg-muted/20' : 'hover:bg-muted/40'}`}
      >
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {viewMode === 'table' ? (
        <div className="space-y-6">
          {/* Seção 1: Fornecedores Ativos */}
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 inline-block"></span>
                <h2 className="text-xs font-bold text-foreground uppercase tracking-wider">
                  Fornecedores Ativos ({ativos.length})
                </h2>
              </div>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {ativos.length === 0 ? (
                    ...
                    ativos.map((fornecedor) => renderFornecedorRow(fornecedor, false))
                  )}
                </TableBody>
              </Table>
            </div>
            ...
            <div className="space-y-2 pt-4 border-t border-dashed">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-muted-foreground">
                  Fornecedores e parceiros desativados
                </span>
              </div>

              <div className="rounded-xl border bg-card overflow-x-auto shadow-xs border-dashed">
                <Table>
                  ...
                  <TableBody>
                    {inativos.map((fornecedor) => renderFornecedorRow(fornecedor, true))}
                  </TableBody>
                </Table>
              </div>
            </div>
        ...
        <div className="space-y-8">
          {filteredData.length === 0 ? (
            ...
            <>
              {/* Seção 1: Fornecedores Ativos em Cards */}
              <div className="space-y-3">
                ...
                {ativos.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {ativos.map((fornecedor) => renderFornecedorCard(fornecedor, false))}
                  </div>
                )}
              </div>

              {/* Seção 2: Fornecedores Inativos em Cards */}
              {inativos.length > 0 && (
                <div className="space-y-3 pt-6 border-t border-dashed">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {inativos.map((fornecedor) => renderFornecedorCard(fornecedor, true))}
                  </div>
                </div>
              )}
            </>
