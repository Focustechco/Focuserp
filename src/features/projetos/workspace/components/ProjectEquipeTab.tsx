...
import { ProjectMember, PapelEquipe } from '../types';
...
interface ProjectEquipeTabProps {
  projeto: Projeto;
  onNavigateTab: (tabId: string) => void;
}

export function ProjectEquipeTab({ projeto }: ProjectEquipeTabProps) {
  const { members, addMember, updateMember, deleteMember } = useProjetoWorkspaceStore(projeto);
  ...
  const [papelNoProjeto, setPapelNoProjeto] = useState<PapelEquipe>('Fullstack Engineer');
  const [email, setEmail] = useState('');
  const [telefone, setTelefone] = useState('');
  const [horasDedicadasSemana, setHorasDedicadasSemana] = useState('40');
  const [permissao, setPermissao] = useState<'Administrador' | 'Gerente' | 'Colaborador' | 'Cliente / Visualizador'>('Colaborador');

  const totalHorasSemanais = members.reduce((acc, m) => acc + (m.horasDedicadasSemana || 0), 0);
  ...
  const getPapelBadge = (papel: PapelEquipe) => {
    switch (papel) {
      ...
      case 'Cliente / Stakeholder': return 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20';
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="rounded-2xl border shadow-xs bg-card">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <div className="text-2xl font-bold text-foreground">{members.length} Profissionais</div>
              <p className="text-[11px] text-muted-foreground">Alocados no projeto {projeto.codigo}</p>
            </div>
        ...
        <Card className="rounded-2xl border shadow-xs bg-card">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground font-medium">Capacidade Produtiva Semanal</span>
              <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{totalHorasSemanais}h / semana</div>
              <p className="text-[11px] text-muted-foreground">Dedicação combinada da equipe</p>
            </div>
        ...
        <Card className="rounded-2xl border shadow-xs bg-card">
          <CardContent className="p-5 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground font-medium">Liderança de Projeto</span>
              <div className="text-sm font-bold text-foreground truncate">{projeto.responsavelPrincipal}</div>
              <p className="text-[11px] text-muted-foreground">Gerente de Projeto / PM</p>
            </div>
        ...
        <CardHeader className="pb-3 border-b bg-muted/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <Users className="w-4 h-4 text-orange-500" /> Squad & Alocação de Recursos
            </CardTitle>
            <CardDescription className="text-xs">
              Membros, papéis técnicos e controle de permissões no projeto.
            </CardDescription>
          </div>
          ...
          <Dialog open={openModal} onOpenChange={setOpenModal}>
            <DialogTrigger asChild>
              <Button className="bg-orange-600 hover:bg-orange-700 text-white rounded-xl gap-1.5 font-bold text-xs h-8 shadow-xs cursor-pointer">
                <Plus className="w-3.5 h-3.5" /> Alocar Profissional
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-md rounded-2xl">
              <DialogHeader>
                <DialogTitle className="text-base font-bold flex items-center gap-2">
                  <Users className="w-4 h-4 text-orange-500" /> Alocar Colaborador no Projeto
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSave} className="space-y-4 pt-2 text-xs">
                <div className="space-y-1.5">
                  <Label className="font-semibold">Selecionar Usuário do ERP (Opcional)</Label>
                  <Select value={selectedUserId} onValueChange={handleSelectUser}>
                    <SelectTrigger className="rounded-xl h-9 text-xs">
                      <SelectValue placeholder="Escolha um usuário cadastrado..." />
                    </SelectTrigger>
                    <SelectContent>
                      {usuarios.map(u => (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Select value={papelNoProjeto} onValueChange={(v: any) => setPapelNoProjeto(v)}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="Cliente / Stakeholder">Cliente / Stakeholder</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={permissao} onValueChange={(v: any) => setPermissao(v)}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="Colaborador">Colaborador (Tasks)</SelectItem>
                        <SelectItem value="Cliente / Visualizador">Cliente (Leitura)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    <Label className="font-semibold">Horas / Semana</Label>
                    <Input 
                      type="number" 
          ...
          {members.length === 0 ? (
            <div className="text-center py-10 border-2 border-dashed rounded-2xl p-6">
              ...
              <p className="text-xs text-muted-foreground mt-1 max-w-sm mx-auto">
                Aloque desenvolvedores, designers e gerentes para este projeto.
              </p>
              <Button 
                  ...
                  <div className="space-y-1.5 pt-1 text-xs">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      <Badge variant="outline" className="text-[10px] gap-1">
                        <Lock className="w-2.5 h-2.5" /> {membro.permissao}
                      </Badge>
                    </div>
