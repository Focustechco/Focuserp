...
interface DetalhesRecebimentoSheetProps {
  titulo: TituloReceber | any | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onBaixar?: (titulo: any) => void;
  onExcluir?: (titulo: any) => void;
}
...
export function DetalhesRecebimentoSheet({
  titulo,
  open,
  onOpenChange,
  onBaixar,
  onExcluir,
}: DetalhesRecebimentoSheetProps) {
  if (!titulo) return null;

  const valorNum = Number(titulo.valorOriginal ?? titulo.valor ?? titulo.saldo ?? 0) || 0;
  const valorRecebido = Number(titulo.valorRecebido ?? 0) || 0;
  const clienteNome = titulo.cliente || titulo.clienteNome || titulo.nomeCliente || 'Cliente não informado';
  const statusNorm = (titulo.status || '').trim().toLowerCase();
  const isPago = statusNorm === 'recebido' || statusNorm === 'liquidado' || statusNorm === 'pago';
  const isVencido = titulo.isVencido ?? false;
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" hideCloseButton className="rounded-t-3xl max-h-[90vh] p-0 bg-background overflow-hidden flex flex-col">
        ...
        <div className="px-5 pt-3 pb-2 border-b bg-muted/20">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              >
                {isPago ? 'Recebido / Liquidado' : isVencido ? 'Vencido' : 'A Vencer / Pendente'}
              </Badge>
              {titulo.numero && (
                <span className="text-xs text-muted-foreground font-mono flex items-center gap-0.5">
                  <Hash className="w-3 h-3" /> {titulo.numero}
                </span>
              )}
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {/* Card Destaque de Valor e Cliente */}
          ...
          <div className="bg-card rounded-2xl border p-4 space-y-2 shadow-2xs">
            ...
            <div className="bg-muted/30 p-3 rounded-xl text-xs text-foreground/90 font-medium leading-relaxed border">
              {titulo.descricao || 'Sem descrição informada.'}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div className="p-2.5 rounded-xl bg-muted/20 border space-y-1">
                ...
                <p className="font-semibold text-foreground truncate">
                  {titulo.categoria || 'Receita Operacional'}
                </p>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-muted/20 border space-y-1">
                ...
                <p className="font-semibold text-foreground truncate">
                  {titulo.centroCustoNome || titulo.centroCusto || 'Geral / Administrativo'}
                </p>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-muted/20 border space-y-1">
                ...
                <p className="font-semibold text-foreground truncate">
                  {titulo.responsavel || 'Operador Financeiro'}
                </p>
              </div>
            </div>
            ...
            <div className="p-3 rounded-xl bg-muted/20 border space-y-2 text-xs">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-bold ${isVencido ? 'text-rose-600' : 'text-foreground'}`}>
                  {titulo.dataVencimento ? formatDateBrasilia(titulo.dataVencimento) : 'Não informado'}
                </span>
              </div>
              ...
              {titulo.dataEmissao && (
                <div className="flex items-center justify-between text-[11px] text-muted-foreground border-t border-dashed pt-1.5">
                  ...
                  <span>{formatDateBrasilia(titulo.dataEmissao)}</span>
                </div>
              )}
              ...
              {titulo.dataRecebimento && (
                <div className="flex items-center justify-between text-[11px] text-emerald-600 dark:text-emerald-400 border-t border-dashed pt-1.5 font-medium">
                  <span className="flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3" /> Recebido em:
                  </span>
                  <span>{formatDateBrasilia(titulo.dataRecebimento)}</span>
                </div>
              )}
            </div>
